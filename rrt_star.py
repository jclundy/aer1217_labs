...
def plan(start, goal, target_gate):
    ...
    for _ in range(ITERATION):
        ...
        best_cost = costs[nearest_idx] + weighted_dist(new_pt, np.array(nodes[nearest_idx]))
        for nb in neighbours:
            c = costs[nb] + weighted_dist(new_pt, np.array(nodes[nb]))
        ...
        ###rewire
        for nb in neighbours:
            c = best_cost + weighted_dist(new_pt, np.array(nodes[nb]))
